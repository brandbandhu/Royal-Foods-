...
export default function GalleryPage() {
  ...
  return (
    <>
      <PageHero
        title="Gallery"
        subtitle="A visual taste of Royal Foods institutional food service, dining operations, kitchens, plates, events and people."
        crumb="Gallery"
      />
      ...
      {open !== null && list[open] && (
        <div
          className="fixed inset-0 z-[60] bg-black/85 backdrop-blur-sm grid place-items-center p-4 animate-fade-in"
          onClick={() => setOpen(null)}
        >
          <button
            onClick={() => setOpen(null)}
            className="absolute top-5 right-5 h-11 w-11 grid place-items-center rounded-full bg-white/10 text-white hover:bg-white/20"
            aria-label="Close"
          >
            <X className="h-5 w-5" />
          </button>
