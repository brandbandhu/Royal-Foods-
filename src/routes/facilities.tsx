...
export const Route = createFileRoute("/facilities")({
  head: () => ({
    meta: [
      { title: "Facilities We Serve | Royal Foods" },
      { name: "description", content: "Royal Foods serves educational institutions with hostel mess management, institutional kitchens, seminar catering, bulk meal supply and quality-controlled dining operations." },
      { property: "og:title", content: "Facilities We Serve | Royal Foods" },
      { property: "og:description", content: "Institutional facilities served by Royal Foods catering." },
...
function FacilitiesPage() {
  ...
  return (
    <>
      <PageHero title="Facilities We Serve" subtitle="Royal Foods supports educational institutions with professional food service operations, hygienic kitchens, and large-scale student dining." crumb="Facilities" />
            ...
            {FACILITIES.map((f, i) => (
              <Reveal key={f.title} delay={i*40}>
                <div className="group hover-lift bg-white border border-border rounded-2xl p-7 h-full min-h-[260px] flex flex-col justify-between shadow-soft">
                  ...
                  <p className="mt-4 text-[0.95rem] leading-relaxed text-muted-foreground">{f.desc}</p>
                </div>
              </Reveal>
            ))}
          </div>
          ...
          {banners.map((b, i) => (
            <Reveal key={b.t} delay={i*100}>
              <div className="img-zoom relative rounded-3xl overflow-hidden shadow-card hover-lift h-72">
                <img src={b.img} alt={b.t} width={1024} height={768} loading="lazy" className="w-full h-full object-cover"/>
      ...
      <section className="py-20">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="rounded-3xl bg-gradient-brand text-white p-10 md:p-14 text-center shadow-glow">
            <h2 className="font-display text-3xl md:text-4xl font-extrabold">Need dependable institutional food service?</h2>
            <p className="mt-3 text-white/90">We tailor menus, staffing, supervision, and schedules to your institution's exact dining needs.</p>
            <Link to="/contact" className="mt-6 inline-flex items-center gap-2 rounded-full bg-white text-primary px-6 py-3 font-semibold hover:scale-105 transition">
